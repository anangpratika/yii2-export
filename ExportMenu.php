...
class ExportMenu extends GridView
{
     ...
     * - `options`: array, the HTML attributes for the toggle label text. Defaults to `['class'=>'an-toggle-all']`
     */
    ...
    protected function initColumnSelector()
    {
        ...
        $header = ArrayHelper::getValue($this->columnSelectorOptions, 'header', Yii::t('anexport', 'Select Columns'));
        $this->columnSelectorOptions['header'] = (empty($header) || $header === false) ? '' :
            '<li class="dropdown-header">' . $header . '</li><li class="an-divider"></li>';
        $id = $this->options['id'] . '-cols';
        Html::addCssClass($this->columnSelectorMenuOptions, 'dropdown-menu an-checkbox-list');
        ...
        $this->columnSelectorOptions = array_replace_recursive([
            ...
            'title' => Yii::t('anexport', 'Select columns to export'),
            'type' => 'button',
    ...
    {
        $label = Yii::t('anexport', 'Column') . ' ' . ($key + 1);
    ...
    public function initExport()
    {
        ...
        if (empty($this->filename)) {
            $this->filename = Yii::t('anexport', 'grid-export');
        }
        $target = $this->target;
        $id = ArrayHelper::getValue($this->exportFormOptions, 'id', $this->options['id'] . '-form');
        Html::addCssClass($this->exportFormOptions, 'an-export-full-form');
    ...
    protected function setDefaultExportConfig()
    {
        ...
        $this->_defaultExportConfig = [
            self::FORMAT_HTML => [
                'label' => Yii::t('anexport', 'HTML'),
                'icon' => $isFa ? 'file-text' : 'floppy-saved',
                'iconOptions' => ['class' => 'text-info'],
                'linkOptions' => [],
                'options' => ['title' => Yii::t('anexport', 'Hyper Text Markup Language')],
                'alertMsg' => Yii::t('anexport', 'The HTML export file will be generated for download.'),
                'mime' => 'text/html',
                'extension' => 'html',
                'writer' => 'HTML',
            ],
            self::FORMAT_CSV => [
                'label' => Yii::t('anexport', 'CSV'),
                'icon' => $isFa ? 'file-code-o' : 'floppy-open',
                'iconOptions' => ['class' => 'text-primary'],
                'linkOptions' => [],
                'options' => ['title' => Yii::t('anexport', 'Comma Separated Values')],
                'alertMsg' => Yii::t('anexport', 'The CSV export file will be generated for download.'),
                'mime' => 'application/csv',
                'extension' => 'csv',
                'writer' => 'CSV',
            ],
            self::FORMAT_TEXT => [
                'label' => Yii::t('anexport', 'Text'),
                'icon' => $isFa ? 'file-text-o' : 'floppy-save',
                'iconOptions' => ['class' => 'text-muted'],
                'linkOptions' => [],
                'options' => ['title' => Yii::t('anexport', 'Tab Delimited Text')],
                'alertMsg' => Yii::t('anexport', 'The TEXT export file will be generated for download.'),
                'mime' => 'text/plain',
                'extension' => 'txt',
                'writer' => 'CSV',
            ],
            self::FORMAT_PDF => [
                'label' => Yii::t('anexport', 'PDF'),
                'icon' => $isFa ? 'file-pdf-o' : 'floppy-disk',
                'iconOptions' => ['class' => 'text-danger'],
                'linkOptions' => [],
                'options' => ['title' => Yii::t('anexport', 'Portable Document Format')],
                'alertMsg' => Yii::t('anexport', 'The PDF export file will be generated for download.'),
                'mime' => 'application/pdf',
                'extension' => 'pdf',
                'writer' => 'PDF',
            ],
            self::FORMAT_EXCEL => [
                'label' => Yii::t('anexport', 'Excel 95 +'),
                'icon' => $isFa ? 'file-excel-o' : 'floppy-remove',
                'iconOptions' => ['class' => 'text-success'],
                'linkOptions' => [],
                'options' => ['title' => Yii::t('anexport', 'Microsoft Excel 95+ (xls)')],
                'alertMsg' => Yii::t('anexport', 'The EXCEL 95+ (xls) export file will be generated for download.'),
                'mime' => 'application/vnd.ms-excel',
                'extension' => 'xls',
                'writer' => 'Excel5',
            ],
            self::FORMAT_EXCEL_X => [
                'label' => Yii::t('anexport', 'Excel 2007+'),
                'icon' => $isFa ? 'file-excel-o' : 'floppy-remove',
                'iconOptions' => ['class' => 'text-success'],
                'linkOptions' => [],
                'options' => ['title' => Yii::t('anexport', 'Microsoft Excel 2007+ (xlsx)')],
                'alertMsg' => Yii::t('anexport', 'The EXCEL 2007+ (xlsx) export file will be generated for download.'),
    ...
    protected function registerAssets()
    {
        ...
        $this->messages += [
            'allowPopups' => '',
            'confirmDownload' => Yii::t('anexport', 'Ok to proceed?'),
            'downloadProgress' => Yii::t('anexport', 'Generating the export file. Please wait...'),
    ...
    public function renderExportMenu()
    {
        ...
        if ($this->asDropdown) {
            ...
            if (empty($this->dropdownOptions['title'])) {
                $this->dropdownOptions['title'] = Yii::t('anexport', 'Export data in selected format');
            }
    ...
    public function initPHPExcel()
    {
        ...
        $title = '';
        $subject = '';
        $description = Yii::t('anexport', 'Grid export generated by Anang Pratika ExportMenu widget (yii2-export)');
        $category = '';
        $keywords = '';
        $manager = '';
        $company = 'Anang Pratika';
        $created = date("Y-m-d H:i:s");
        $lastModifiedBy = 'anangpratika';
